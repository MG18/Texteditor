public class Add extends Commands{
    ...
    public void add(Document document, Integer index, String text) {
        ...
        if (document.checkDocumentEmpty()) {
            document.getDocument().put(1, text);

        } else if (checkIndexNull(index) || index > document.getDocument().lastKey()) {
            document.getDocument().put(document.getDocument().lastKey()+1, text);
        }
            ...
            document.getDocument().put(index, text);
        }
